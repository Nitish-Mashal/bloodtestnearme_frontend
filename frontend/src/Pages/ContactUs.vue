            ...
            <div class="md:px-5 max-w-md">
              ...
              >
                ...
                <el-form-item label="How did you find us?" prop="source">
                  <el-select v-model="form.source" placeholder="Select option">
  ...
  contactForm.value.validate(async (valid) => {
    ...
    try {
      ...
      if (data.message?.includes("success")) {
        successMsg.value = data.message || "✅ Message sent successfully!";
        form.value = { name: "", email: "", phone: "", source: "" };
      } else {
        errorMsg.value = data.message || "❌ Something went wrong.";
      }
    } catch (error) {
      console.error("⚠️ Error posting contact form:", error);
      errorMsg.value = "⚠️ Something went wrong. Please try again.";
    } finally {
      loading.value = false;
    }
  });
