            ...
            <div class="p-2 sm:p-3 rounded-xl">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
                    <div v-for="(pkg, index) in filteredPackages" :key="index"
                        class="bg-white rounded-xl overflow-hidden border border-gray-200 shadow-sm hover:shadow-lg transition-shadow cursor-pointer">
...
const fetchPackages = async () => {
    try {
        ...
        if (response.data && response.data.message && response.data.message.data) {
            packages.value = response.data.message.data; // ✅ Corrected
        }
    } catch (error) {
