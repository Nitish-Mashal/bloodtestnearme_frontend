            ...
            <div v-if="filteredPackages.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
                <div v-for="pkg in filteredPackages" :key="pkg.name1"
                    ...
                    1-1.5 0 .75.75 0 0 1 1.5 0Z" />
                        ...
                        <!-- Action Buttons -->
                        <div
                            class="flex flex-col sm:flex-row sm:justify-between items-stretch sm:items-center gap-2 mt-2">
                            ...
                            <router-link :to="`/${pkg.url}`" class="w-full sm:w-auto no-underline">
                                <button
                                    class="border-1 border-[#001D55] font-semibold text-sm bold-test-color px-3 py-1.5 rounded-full hover:bg-gray-100 transition flex items-center justify-center gap-1 whitespace-nowrap w-full sm:w-auto">
